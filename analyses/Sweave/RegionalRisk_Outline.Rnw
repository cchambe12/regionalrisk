...
\item We used the daily minimum temperature dataset to determine if a false spring occurred, which was defined as -2.2$^{\circ}$C \citep{Schwartz1993}
...
\item Mean spring temperature was calculated -- after chilling was accummulated -- to incorporate spring forcing temperatures in our Bayesian hierarchichal model and to compare differences in spring across sites \citep{Basler2012}.
...
\item We fit a mixed-effects model using mean spring temperature, NAO, elevation, space, and climate change as predictors (fixed effects) and species as modeled groups (random effects).
...
y_i \thicksim N(\alpha_(i) + \beta_{MeanSpringTemp_{sp_{(i)}}} + \beta_{NAO_{sp_{(i)}}} + \beta_{elevation_{sp_{(i)}}} + \beta_{space_{sp_{(i)}}} \beta_{ClimateChange_{sp_{(i)}}} \sigma_{sp_{(i)}} 
\end{equation}
