...
\item In Germany, for example, the last freeze date has advanced by 2.6 days per decade since 1955 \citep{Zohner2016} but leafout is advancing around twice as fast. %EMW: Compared to?
...
\item The North Atlantic Ossilation (NAO) index is often used to describe winter and spring circulation across Europe. %EMW: Need better transition from above paragraph. 
\begin{enumerate}
...
\item The majority of false spring studies assess the effects of one predictor (e.g. elevation or distance from the coast) on false spring prevalence but most fail to incorporate multiple effects. 
\begin{enumerate}
...
{\begin{figure} [H]
  -\begin{center}
  -\includegraphics[width=12cm]{..//figures/Boxplot_BBTminFS_perDecade.pdf}
