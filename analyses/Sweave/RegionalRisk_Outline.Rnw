...
$^{4}$Edificio Ciencias, Campus Universitario 28805 AlcalÃ¡ de Henares, Madrid, Spain \\
\vspace{2ex}
...
%\item Plant phenology, which is defined as the timing of recurring life-history events such as budburst, strongly tracks shifts in climate \citep{Cleland2007, Wolkovich2012}. %EMW: this not really the topic of any of the below, I think you can actually just cut this from the paper and replace it with a topic sentence that relates to the below: Frost can have big impacts on plants and plants appear least frost resistant in one period. See if the below suggestion works .... 
...
%IMC - General comment on the intro: 
% 1) I'm not sure if the intro needs restructuring. As is, it starts listing several FS effects and patterns before defining FS, its relevance and how we still don't know much about the topic (done in the 2nd paragraph). In my mind it would make more sense to shift the 2nd by the 1st paragraph, but I can be convinced otherwise.
% 2) Perhaps once fully written, the intro would be a bit too long (but sure you do a great job at streamlining). In the case anything needs to go I'd suggest paragraph 3 because this paper is not about cues, righ? 
...
\item We used a space parameter, rather than a more traditional latitude parameter, to adjust for spatial autocorrelation issues using a minimization of Moran's \textit{I} of the residuals \citep{Baumen2017} (Figure S1). % Regarding Moran's I minimization
...
\end{enumerate} % IMC - Perhaps we should have a subsection in the Supplement where we explain in detail why/how we obtain and parameterize our proxy for spatial structure? Also,  in that supplement I would include a map or an additional analysis to interpret the spatial predictor (what does it mean to have a positive slope?). I'm happy to help with this, writing a couple paragraphs explaining the rationale, what do we mean by spatial autocorrelation issues, and so on. 
...
{\begin{figure} [H]
  ...
  -\end{center}
  -\end{figure}}
  %% IMC - I would try to make the dots smaller, I'm aware there are buzillion points and so it's not easy that the cloud of dots reflects the tendency lines. An alternative would be to bin the dots into consecutive boxplots at regular steps in the x-axis (e.g. every 5 years?).
