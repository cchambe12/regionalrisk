...
\item We then calculated budburst by subtracting 16 days from leafout for \textit{Aesculus hippocastenum} and \textit{Alnus glutinosa}, 15 days for \textit{Betula pendula}, 8 days for \textit{Fagus sylvatica}, and 12 days for \textit{Fraxinus excelsior} and \textit{Quercus robur} \citep{Flynn2018}.
\end{enumerate}
