...
all_dist <- ggplot(me.species, aes(x=distkm, y=.prediction, col=species, linetype=as.factor(cc))) + 
  stat_smooth(method="lm", span=0.9, se=TRUE, aes(fill=species, linetype=as.factor(cc)), show.legend=FALSE) +
  stat_smooth(method="lm", span=0.9, se=FALSE, aes(fill=species, linetype=as.factor(cc))) +
  theme_classic() +
  ...
  guides(fill=FALSE, color=FALSE) + scale_linetype_manual(name="Climate Change", values=c("solid", "dotted"),
                                             labels=c("0"="1950-1983",
                                                      "1"="1984-2016")) + 
  theme(legend.key = element_blank(), legend.text.align = 0, legend.position = c(0.85, 0.85)) +
  scale_fill_manual(name="Species", values=cols,
                    labels=c("AESHIP"=expression(paste(italic("Aesculus hippocastanum"))),
                             "ALNGLU"=expression(paste(italic("Alnus glutinosa"))),
                             "aaBETPEN"=expression(paste(italic("Betula pendula"))),
...
all_nao <- ggplot(me.species, aes(x=nao, y=.prediction, col=species)) + 
  stat_smooth(method="lm", span=0.9, se=TRUE, aes(fill=species, linetype=as.factor(cc)), show.legend=FALSE) +
  stat_smooth(method="lm", span=0.9, aes(fill=species, linetype=as.factor(cc)), se=FALSE) +
  scale_colour_manual(name="Species", values=cols,
                      labels=c("AESHIP"=bquote(paste(italic("Aesculus hippocastanum"))),
                               "ALNGLU"=bquote(paste(italic("Alnus glutinosa"))),
                               "aaBETPEN"=bquote(paste(italic("Betula pendula"))),
                               "FAGSYL"=bquote(paste(italic("Fagus sylvatica"))),
                               "zFRAEXC"=bquote(paste(italic("Fraxinus excelsior"))),
                               "QUEROB"=bquote(paste(italic("Quercus robur"))))) +
  xlab("NAO Index") + ylab("Probability of False Spring") +
  guides(fill=FALSE, linetype=FALSE, color=FALSE) + scale_linetype_manual(name="Climate Change", values=c("solid", "dotted"),
                                             labels=c("0"="1950-1983",
                                                      "1"="1984-2016")) + 
  theme(legend.key = element_rect(color = "transparent", fill="white"),
        legend.text.align = 0, legend.position="none") + theme_classic() +
    scale_fill_manual(name="Species", values=cols,
                    labels=c("AESHIP"=bquote(paste(italic("Aesculus hippocastanum"))),
                             "ALNGLU"=bquote(paste(italic("Alnus glutinosa"))),
                             "aaBETPEN"=bquote(paste(italic("Betula pendula"))),
...
spplegend<-g_legend(all_nao)
#cclegend<-g_legend(all_nao)


quartz()
g1 <- grid.arrange(all_mat, all_dist, ncol=2)
g2 <- grid.arrange(all_elev, all_nao, spplegend, ncol=3, widths=c(1, 1, 0.55))
...
##### Now just FRAEXC and BETPEN
all_elev <- ggplot(me.bpfr, aes(x=elev, y=.prediction, col=species, linetype=as.factor(cc))) + 
  stat_smooth(method="lm", span=0.9, se=TRUE, aes(fill=species, linetype=as.factor(cc))) +
  theme_classic() +
  scale_colour_manual(name="Species", values=cols,
...
all_mat <- ggplot(me.bpfr, aes(x=mst, y=.prediction, col=species, linetype=as.factor(cc))) + 
  stat_smooth(method="lm", span=0.9, se=TRUE, aes(fill=species, linetype=as.factor(cc))) +
  theme_classic() +
  scale_colour_manual(name="Species", values=cols,
                      labels=c("BETPEN"=expression(paste(italic("Betula pendula"))),
...
all_dist <- ggplot(me.bpfr, aes(x=distkm, y=.prediction, col=species, linetype=as.factor(cc))) + 
  stat_smooth(method="lm", span=0.9, se=TRUE, aes(fill=species, linetype=as.factor(cc)), show.legend=FALSE) +
  stat_smooth(method="lm", span=0.9, se=FALSE, aes(fill=species, linetype=as.factor(cc))) +
  theme_classic() +
  scale_color_manual(name="Species", values=cols,
...
all_nao <- ggplot(me.bpfr, aes(x=nao, y=.prediction, col=species)) + 
  stat_smooth(method="lm", span=0.9, se=TRUE, aes(fill=species, linetype=as.factor(cc)), show.legend=FALSE) +
  stat_smooth(method="lm", span=0.9, aes(fill=species, linetype=as.factor(cc)), se=FALSE) +
  scale_colour_manual(name="Species", values=cols,
                      labels=c("BETPEN"=bquote(paste(italic("Betula pendula"))),
                               "FRAEXC"=bquote(paste(italic("Fraxinus excelsior"))))) +
  xlab("NAO Index") + ylab("Probability of False Spring") +
  guides(fill=FALSE, linetype=FALSE, color=FALSE) + scale_linetype_manual(name="Climate Change", values=c("solid", "dotted"),
                                                                          labels=c("0"="1950-1983",
                                                                                   "1"="1984-2016")) + 
