...
aes<-read.csv("output/aeship_longdata.csv", header=TRUE)
ag<-read.csv("output/alnglu_longdata.csv", header=TRUE)
bp<-read.csv("output/betpen_longdata.csv", header=TRUE)
fsyl<-read.csv("output/fagsyl_longdata.csv", header=TRUE)
fex<-read.csv("output/fraexc_longdata.csv", header=TRUE)
qr<-read.csv("output/querob_longdata.csv", header=TRUE)

aes$lat.long<-paste(aes$lat, aes$long)
#aes$lo<-ave(aes$doy, aes$lat.long, aes$year, FUN=last)
#aes$bb<-aes$lo-24 # Based on Danf's BETPAP - most closely related. Choose WL0 based on keeping all species consistent
...
#ag$bb<-ag$lo-24 # Based on Danf's ALNINC - most closely related. Choose WL0 based on keeping all species consistent
#ag<-ag[!(ag$doy<ag$bb),]
#ag<-ag[!duplicated(ag),]
ag<-na.omit(ag)
...
#bp$bb<-bp$lo-24 # Based on Danf's BETPAP - most closely related. Choose WL0 based on keeping all species consistent
#bp<-bp[!(bp$doy<bp$bb),]
#bp<-bp[!duplicated(bp),]
bp<-na.omit(bp)
...
#fsyl$bb<-fsyl$lo-24 # Based on Danf's FAGGRA - most closely related. Choose WL0 based on keeping all species consistent
...
#fex$bb<-fex$lo-24 # Based on Danf's FRANIG - most closely related. Choose WL0 based on keeping all species consistent
...
#qr$bb<-qr$lo-24 # Based on Danf's QUEALB - most closely related. Choose WL0 based on keeping all species consistent
#qr<-qr[!(qr$doy<qr$bb),]
#qr<-qr[!duplicated(qr),]
qr<-na.omit(qr)
...
write.csv(d, file="~/Documents/git/regionalrisk/analyses/output/tminprep_boxplots_long.csv", row.names=FALSE)
