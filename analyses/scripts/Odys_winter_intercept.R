...
bb.stan<-bb.stan[sample(nrow(bb.stan), 100000), ]

brm.full.nointer<-brm(fs.count~m.index+sp.temp+cc+sm.elev+space+m.index:cc + sp.temp:cc +sm.elev:cc +
                        space:cc+
                        (1|species), data=bb.stan, chains=4,cores=4)

save(brm.full.nointer, file="/n/wolkovich_lab/Lab/Cat/brm_winter_intercept.Rdata")
