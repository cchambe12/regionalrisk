...
df$bb<-ifelse(df$species=="BETPEN", df$lo-11, df$bb) #was 15; used Danf's WL1 and CS0 data average for all Betula species
...
dxx$species<-as.numeric(substrRight(dxx$pep.year, 6))
...
write.csv(dxx, file="~/Desktop/allspp_climateprep_dvr.csv", row.names=FALSE)
