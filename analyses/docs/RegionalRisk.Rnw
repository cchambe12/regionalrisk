...
+ \beta_{Elevation \times ClimateChange_{(i)}} \\ +& \beta_{DistanceCoast \times ClimateChange_{(i)}} + \beta_{Space \times ClimateChange_{(i)}} + \sigma_{(i)}, \\   
\end{align*}
%\item The $\beta$ coefficients and $\alpha$ were modeled at the species level:
{\setlength{\mathindent}0cm}
\begin{align*}
y_{(i)} \thicksim Bernoulli(0,1)
