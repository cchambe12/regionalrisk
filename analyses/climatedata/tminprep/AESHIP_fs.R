...
d<-read.csv("/n/wolkovich_lab/Lab/Cat/bbch_region_aesculus.csv", header=TRUE)
...
df<-d%>%
  ...
  rename(long=LON)
df$bb<-df$mid-12
df$lo<-df$mid+12## instead of 12; used Danf's data for Acer species WL1 and CS0 to find average 
...
write.csv(aeship, file="/n/wolkovich_lab/Lab/Cat/aeship_longdata.csv", row.names=FALSE)
