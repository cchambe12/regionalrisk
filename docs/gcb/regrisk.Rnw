...
\section*{Introduction} %% 5507 words, 8000 word limit
...
Here we investigate the influence of known climatic and geographic factors on false spring risk. %Removed because we define this more clearly in the methods.\citep[defined here as when temperatures fell below -2.2$^{\circ}$ between estimated budburst and leafout,][]{Schwartz1993}. 
