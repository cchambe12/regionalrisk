...
% EMW: Would need to adjust below if we decide to cut combined geo/clim+species estimates. 
%% NEED TO FIX!!! %%
Okay, we're trying to say that some species are better captured by the climatic and geographic factors included in the model (i.e., \textit{Fagus sylvactica}), whereas others have large residual climate change effects, unexplained by the other model predictors (i.e., XXX)
