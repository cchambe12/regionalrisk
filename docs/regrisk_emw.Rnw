...
In general, it is important to consider the effects of climate change on both budburst and leafout, the timing when individuals are most at risk to spring freeze damage \citep{Chamberlain2019,Lenz2016} though we found that differing rates of budburst across species had minimal effects on predicting risk whereas a lower temperature threshold may have a broader impact on model predictability. Species phenological differences did change the level of risk with climate change. We see that climate change is changing the way certain predictors affect false spring risk than they did before especially with mean spring temperature and NAO. 
