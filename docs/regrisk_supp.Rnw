...
load("bbmod.Rdata")
bb <- as.data.frame(tidy(bb.mod, prob=0.98))
bb$term <- gsub("b_", "", bb$term)
bb$term <- gsub("Intercept", "Intercept", bb$term)
bb$term <- gsub("cc", "Climate Change", bb$term)
bb$term <- gsub("speciesALNGLU", paste0("\\\\textit{","Alnus glutinosa","}"), bb$term)
bb$term <- gsub("speciesBETPEN", paste0("\\\\textit{","Betula pendula","}"), bb$term)
bb$term <- gsub("speciesFAGSYL", paste0("\\\\textit{","Fagus sylvatica","}"), bb$term)
bb$term <- gsub("speciesFRAEXC", paste0("\\\\textit{","Fraxinus excelsior","}"), bb$term)
bb$term <- gsub("speciesQUEROB", paste0("\\\\textit{","Quercus robur","}"), bb$term)
bb$term <- gsub(":", " x ", bb$term)

colnames(bb) <- c("", "mean", "std.err", "2\\%", "98\\%")
print(xtable(bb[1:12,c(1:2,4:5)], caption = "Summary of simple linear regression model of day of budburst before and after climate change across species.", label="tab:simbbmod",
...
<<label=simtim, results="asis", echo=FALSE, warning=FALSE, message=FALSE, fig.pos="H", fig.width=7, fig.height=4, fig.align='center'>>=
setwd("~/Documents/git/regionalrisk")
load("tminmod.Rdata")
tmin <- as.data.frame(tidy(tmin.mod, prob=0.98))

tmin$term <- gsub("b_", "", tmin$term)
tmin$term <- gsub("Intercept", "Intercept", tmin$term)
tmin$term <- gsub("cc", "Climate Change", tmin$term)
tmin$term <- gsub("speciesALNGLU", paste0("\\\\textit{","Alnus glutinosa","}"), tmin$term)
tmin$term <- gsub("speciesBETPEN", paste0("\\\\textit{","Betula pendula","}"), tmin$term)
tmin$term <- gsub("speciesFAGSYL", paste0("\\\\textit{","Fagus sylvatica","}"),tmin$term)
tmin$term <- gsub("speciesFRAEXC", paste0("\\\\textit{","Fraxinus excelsior","}"), tmin$term)
tmin$term <- gsub("speciesQUEROB", paste0("\\\\textit{","Quercus robur","}"), tmin$term)
tmin$term <- gsub(":", " x ", tmin$term)


colnames(tmin) <- c("", "mean", "std.err", "2\\%", "98\\%")
print(xtable(tmin[1:12,c(1:2, 4:5)], caption = "Summary of simple linear regression model of average minimum temperature between budburst and leafout before and after climate change across species.", label="tab:simptmin",
             align = c("l", "l", "r","r","r")),  #>{\\textit}l
      include.rownames=F,
      caption.placement = "top", 
      sanitize.text.function = function(x) {x}, table.placement="H")
...
<<label=simfs, results="asis", echo=FALSE, warning=FALSE, message=FALSE, fig.pos="H", fig.width=7, fig.height=4, fig.align='center'>>=
setwd("~/Documents/git/regionalrisk")
if(TRUE){
load("fstotmod.Rdata")
lstfrz <- as.data.frame(tidy(fstot.mod, prob=0.98))
lstfrz$term <- gsub("b_", "", lstfrz$term)
lstfrz$term <- gsub("Intercept", "Intercept", lstfrz$term)
lstfrz$term <- gsub("cc", "Climate Change", lstfrz$term)
lstfrz$term <- gsub("speciesALNGLU", paste0("\\\\textit{","Alnus glutinosa","}"), lstfrz$term)
lstfrz$term <- gsub("speciesBETPEN", paste0("\\\\textit{","Betula pendula","}"), lstfrz$term)
lstfrz$term <- gsub("speciesFAGSYL", paste0("\\\\textit{","Fagus sylvatica","}"), lstfrz$term)
lstfrz$term <- gsub("speciesFRAEXC", paste0("\\\\textit{","Fraxinus excelsior","}"), lstfrz$term)
lstfrz$term <- gsub("speciesQUEROB", paste0("\\\\textit{","Quercus robur","}"), lstfrz$term)
lstfrz$term <- gsub(":", " x ", lstfrz$term)
colnames(lstfrz) <- c("", "mean", "std.err", "2\\%", "98\\%")
print(xtable(lstfrz[1:12,c(1:2,4:5)], caption = "Summary of simple linear regression model of the number of false springs before and after climate change across species.", label="tab:simpfs",
             align = c("l", "l", "r","r","r")),  #>{\\textit}l
      include.rownames=F, 
      caption.placement = "top", 
      sanitize.text.function = function(x) {x}, table.placement="H")
...
modoutput <- read.csv("output/long_full_modeloutput.csv", header=TRUE)
modoutput <- modoutput[c(1:47),]
...
modoutput$term <- gsub("nao.z", "NAO Index", modoutput$term)
modoutput$term <- gsub("mat.z", "Mean Spring \nTemperature", modoutput$term)
modoutput$term <- gsub("elev.z", "Elevation", modoutput$term)
modoutput$term <- gsub("dist.z", "Distance from \nCoast", modoutput$term)
modoutput$term <- gsub("space.z", "Space Parameter", modoutput$term)
modoutput$term <- gsub("cc.z", "Climate Change", modoutput$term)
modoutput$term <- gsub(":Climate Change", " x Climate Change", modoutput$term)
#modoutput$term <- gsub(":", " x ", modoutput$term)

modoutput$term <- gsub(":speciesALNGLU", paste0("\nx","\\\\textit{", " Alnus glutinosa","}"), modoutput$term)
modoutput$term <- gsub(":speciesBETPEN", paste0("\nx","\\\\textit{"," Betula pendula","}"), modoutput$term)
modoutput$term <- gsub(":speciesFAGSYL", paste0("\nx","\\\\textit{"," Fagus sylvatica","}"), modoutput$term)
modoutput$term <- gsub(":speciesFRAEXC", paste0("\nx","\\\\textit{"," Fraxinus excelsior","}"), modoutput$term)
modoutput$term <- gsub(":speciesQUEROB", paste0("\nx","\\\\textit{"," Quercus robur","}"), modoutput$term)

modoutput$term <- gsub("speciesALNGLU", paste0("\\\\textit{","Alnus glutinosa","}"), modoutput$term)
modoutput$term <- gsub("speciesBETPEN", paste0("\\\\textit{","Betula pendula","}"), modoutput$term)
modoutput$term <- gsub("speciesFAGSYL", paste0("\\\\textit{","Fagus sylvatica","}"), modoutput$term)
modoutput$term <- gsub("speciesFRAEXC", paste0("\\\\textit{","Fraxinus excelsior","}"), modoutput$term)
modoutput$term <- gsub("speciesQUEROB", paste0("\\\\textit{","Quercus robur","}"), modoutput$term)

colnames(modoutput) = c("", "mean", "2\\%", "10\\%", "25\\%", "75\\%", "90\\%", "98\\%")

print(xtable(modoutput, 
             caption = "Summary of Bernoulli model with the effects of species, climatic and geographical predictors on false spring risk.", label="tab:suppmodlong",
...
{\begin{figure} [H]
  ...
  -\caption{Effects of species, climatic and geographical predictors on false spring risk with different rates of leafout for each species. More positive values indicate an increased probability of a false spring whereas more negative values suggest a lower probability of a false spring. Dots and lines show means and 98\% uncertainty intervals. See Table \ref{tab:suppmodlong} for full model output. }\label{fig:dvr}
  -\end{center}
  -\end{figure}}
  
\newpage
<<label=suppmodlong, results="asis", echo=FALSE, warning=FALSE, message=FALSE, fig.pos="H", fig.width=7, fig.height=4, fig.align='center'>>=
setwd("..//analyses/")

modoutput <- read.csv("output/dvrlong_full_modeloutput.csv", header=TRUE)
modoutput <- modoutput[c(1:47),]
#modoutput <- subset(modoutput, select=c("term", "estimate", "10%", "50%", "90%"))
# add multicolumn later for BB and LO headers above these column names
...
print(xtable(modoutput, 
             caption = "Summary of Bernoulli model of false spring risk with different rates of leafout for each species.", label="tab:suppmoddvrlong",
             align = c("l", "l", "r", "r","r","r", "r","r", "r")), #>{\\textit}l
      include.rownames=F, 
      caption.placement = "top", 
      #sanitize.text.function = function(x) {x}, table.placement="H",
      tabular.environment = "longtable", floating = FALSE, #align=c(rep('|c', ncol(sp.table)),'p{4in}'), 
      tabular="\\textwidth",include.colnames = TRUE, add.to.row = list(pos = list(0),command = "\\hline \\endhead "),sanitize.text.function = identity)
@
...
\newpage
<<label=suppmoddvrtemps, results="asis", echo=FALSE, warning=FALSE, message=FALSE, fig.pos="H", fig.width=7, fig.height=4, fig.align='center'>>=
setwd("..//analyses/")

modoutput <- read.csv("output/fivelong_full_modeloutput.csv", header=TRUE)
modoutput <- modoutput[c(1:47),]
#modoutput <- subset(modoutput, select=c("term", "estimate", "10%", "50%", "90%"))
# add multicolumn later for BB and LO headers above these column names
...
print(xtable(modoutput, 
             caption = "Summary of Bernoulli model of false spring risk with a lower temperature threshold (-5$^{\circ}$C) for defining a false spring.", label="tab:suppmodfivelong",
             align = c("l", "l", "r", "r","r","r", "r","r", "r")), #>{\\textit}l
      include.rownames=F, 
      caption.placement = "top", 
      #sanitize.text.function = function(x) {x}, table.placement="H",
...
modoutput <- read.csv("output/longtemps_full_modeloutput.csv", header=TRUE)
modoutput <- modoutput[c(1:47),]
#modoutput <- subset(modoutput, select=c("term", "estimate", "10%", "50%", "90%"))
# add multicolumn later for BB and LO headers above these column names
...
print(xtable(modoutput, 
             caption = "Summary of Bernoulli model of false spring risk with with a lower temperature threshold (-5$^{\circ}$C) for early-leafout species (i.e., \textit{Aesculus hippocastanum}, \textit{Alnus glutinosa} and \textit{Betula pendula}) and a higher temperature threshold (-2.2$^{\circ}$C) for late-leafout species (i.e., \textit{Fagus sylvatica}, \textit{Fraxinus excelsior} and \textit{Quercus robur}).", label="tab:suppmodlongtemps",
             align = c("l", "l", "r", "r","r","r", "r","r", "r")), #>{\\textit}l
      include.rownames=F, 
      caption.placement = "top", 
      #sanitize.text.function = function(x) {x}, table.placement="H",
