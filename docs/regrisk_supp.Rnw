...
load("bbmod.scaled.Rdata")
bb <- as.data.frame(tidy(bb.mod.scaled, prob=0.9))
bb$term <- ifelse(bb$term=="b_Intercept", "Intercept", "Climate Change")
colnames(bb) <- c("", "Estimate", "std.err", "10\\%", "90\\%")
print(xtable(bb[1:2,c(1:2,4:5)], caption = "Summary of simple linear regression model of day of budburst before and after climate change.",
             align = c("l", "l", "r","r","r")), #>{\\textit}l
      include.rownames=F, 
      caption.placement = "top", 
      sanitize.text.function = function(x) {x}, table.placement="H")

load("lstfrz.scaled.Rdata")
lstfrz <- as.data.frame(tidy(lstfrz.mod.scaled, prob=0.9))
lstfrz$term <- ifelse(lstfrz$term=="b_Intercept", "Intercept", "Climate Change")
colnames(lstfrz) <- c("", "Estimate", "std.err", "10\\%", "90\\%")
print(xtable(lstfrz[1:2,c(1:2,4:5)], caption = "Summary of simple linear regression model of day of year of last freeze (-2.2°C) before and after climate change.",
             align = c("l", "l", "r","r","r")), #>{\\textit}l
      include.rownames=F, 
      caption.placement = "top", 
      sanitize.text.function = function(x) {x}, table.placement="H")

load("tmin.simple.Rdata")
tmin <- as.data.frame(tidy(tmin.simple, prob=0.9))
tmin$term <- ifelse(tmin$term=="b_Intercept", "Intercept", "Climate Change")
colnames(tmin) <- c("", "Estimate", "std.err", "10\\%", "90\\%")
print(xtable(tmin[1:2,c(1:2, 4:5)], caption = "Summary of simple linear regression model of average minimum temperature between budburst and leafout before and after climate change.",
             align = c("l", "l", "r","r","r")), #>{\\textit}l
      include.rownames=F, 
      caption.placement = "top", 
      sanitize.text.function = function(x) {x}, table.placement="H")
...
modoutput <- read.csv("output/modoutput_90_origspp.csv", header=TRUE)
modoutput <- subset(modoutput, select=c("termclean", "estclean", "lowclean", "highclean", "species"))

##### VERY CLOSE JUST NEED TO REORDER EVERYTHING! EITHER THE TABLE OR THE FIGURE! #####
modoutput <- modoutput[
  order( modoutput$termclean, modoutput$species ),
] 

modoutput$termclean <- gsub("nao.z", "NAO Index", modoutput$termclean)
modoutput$termclean <- gsub("mat.z", "Mean Spring \nTemperature", modoutput$termclean)
modoutput$termclean <- gsub("elev.z", "Elevation", modoutput$termclean)
modoutput$termclean <- gsub("dist.z", "Distance from \nCoast", modoutput$termclean)
modoutput$termclean <- gsub("space.z", "Space Parameter", modoutput$termclean)
modoutput$termclean <- gsub("cc.z", "Climate Change", modoutput$termclean)
modoutput$termclean <- gsub(":cc.z", " by Climate Change", modoutput$termclean)

modoutput$species <- gsub("aaall", "Overall Estimate", modoutput$species)
modoutput$species <- gsub("AESHIP", "Aesculus hippocastanum", modoutput$species)
modoutput$species <- gsub("ALNGLU", "Alnus glutinosa", modoutput$species)
modoutput$species <- gsub("aaBETPEN", "Betula pendula", modoutput$species)
modoutput$species <- gsub("FAGSYL", "Fagus sylvatica", modoutput$species)
modoutput$species <- gsub("zFRAEXC", "Fraxinus excelsior", modoutput$species)
modoutput$species <- gsub("QUEROB", "Quercus robur", modoutput$species)

colnames(modoutput) = c("", "Estimate", "10\\%", "90\\%", "Species")
