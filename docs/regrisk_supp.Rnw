...
lstfrz <- as.data.frame(tidy(fssimp.mod, prob=0.98))
...
modoutput$term <- gsub(":cc.z", " x Climate Change", modoutput$term)

modoutput$term <- gsub(":speciesALNGLU", "\nx Alnus glutinosa", modoutput$term)
modoutput$term <- gsub(":speciesBETPEN", "\nx Betula pendula", modoutput$term)
modoutput$term <- gsub(":speciesFAGSYL", "\nx Fagus sylvatica", modoutput$term)
modoutput$term <- gsub(":speciesFRAEXC", "\nx Fraxinus excelsior", modoutput$term)
modoutput$term <- gsub(":speciesQUEROB", "\nx Quercus robur", modoutput$term)
