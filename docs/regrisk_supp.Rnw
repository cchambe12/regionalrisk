...
load("bbmod.Rdata")
bb <- as.data.frame(tidy(bb.mod, prob=0.9))
...
bb$term <- gsub(":", " by ", bb$term)

colnames(bb) <- c("", "Estimate", "std.err", "10\\%", "90\\%")
...
load("tminmod.Rdata")
tmin <- as.data.frame(tidy(tmin.mod, prob=0.9))
...
tmin$term <- gsub(":", " by ", tmin$term)

colnames(tmin) <- c("", "Estimate", "std.err", "10\\%", "90\\%")
...
lstfrz <- as.data.frame(tidy(fssimp.mod, prob=0.9))
...
lstfrz$term <- gsub(":", " by ", lstfrz$term)
colnames(lstfrz) <- c("", "Estimate", "std.err", "10\\%", "90\\%")
...
modoutput$term <- gsub("b_Intercept", "Intercept", modoutput$term)
...
modoutput$term <- gsub(":speciesALNGLU", "\nby Alnus glutinosa", modoutput$term)
modoutput$term <- gsub(":speciesBETPEN", "\nby Betula pendula", modoutput$term)
modoutput$term <- gsub(":speciesFAGSYL", "\nby Fagus sylvatica", modoutput$term)
modoutput$term <- gsub(":speciesFRAEXC", "\nby Fraxinus excelsior", modoutput$term)
modoutput$term <- gsub(":speciesQUEROB", "\nby Quercus robur", modoutput$term)

modoutput$term <- gsub("b_speciesALNGLU", "Alnus glutinosa", modoutput$term)
modoutput$term <- gsub("b_speciesBETPEN", "Betula pendula", modoutput$term)
modoutput$term <- gsub("b_speciesFAGSYL", "Fagus sylvatica", modoutput$term)
modoutput$term <- gsub("b_speciesFRAEXC", "Fraxinus excelsior", modoutput$term)
modoutput$term <- gsub("b_speciesQUEROB", "Quercus robur", modoutput$term)

colnames(modoutput) = c("", "Estimate", "10\\%", "25\\%", "75\\%", "90\\%")
