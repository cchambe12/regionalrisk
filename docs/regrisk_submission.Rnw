...
fs <- read.csv("analyses/output/fs_newspace_longtemps.csv", header=TRUE)
fslong <- read.csv("analyses/output/fs_newspace_long.csv", header=TRUE)
fsdvr <- read.csv("analyses/output/fs_newspace_dvrtemps.csv", header=TRUE)
...
mat<-round((mean(fixef(longtemps.full, pars="mat.z", summary=FALSE))/4)/(sd(fs$mst)*2)*100*2, digits=2) ## -7.638351 (vs -3.331)
nao<-round((mean(fixef(longtemps.full, pars="nao.z", summary=FALSE))/4)/(sd(fs$nao)*2)*100*0.3, digits=2) ## 1.915104 (vs 1.54)
dist<-round((mean(fixef(longtemps.full, pars="dist.z", summary=FALSE))/4)/(sd(fs$distkm)*2)*100*150, digits=2) ## 5.320474 (vs 4.83)
elev<-round((mean(fixef(longtemps.full, pars="elev.z", summary=FALSE))/4)/(sd(fs$elev)*2)*100*200, digits=2)## 2.235522 (vs 1.89)

matz<-round((mean(fixef(longtemps.full, pars="mat.z", summary=FALSE))), digits=2)
naoz<-round((mean(fixef(longtemps.full, pars="nao.z", summary=FALSE))), digits=2)
distz<-round((mean(fixef(longtemps.full, pars="dist.z", summary=FALSE))), digits=2)
elevz<-round((mean(fixef(longtemps.full, pars="elev.z", summary=FALSE))), digits=2)


aescc<-round((mean(fixef(longtemps.full, pars="cc.z", summary=FALSE))/4)/(sd(fs$cc)*2)*100,digits=2) ## 8.842486 (vs 7.225) - For AESHIP (residual not explained by regional/climatic factors)
...
aesccz <- round((mean(fixef(longtemps.full, pars="cc.z", summary=FALSE))), digits=2)
alnccz <- round((mean(fixef(longtemps.full, pars="cc.z", summary=FALSE))+mean(fixef(longtemps.full, pars="speciesALNGLU", summary=FALSE))), digits=2)
betccz <- round((mean(fixef(longtemps.full, pars="cc.z", summary=FALSE))+mean(fixef(longtemps.full, pars="speciesBETPEN", summary=FALSE))), digits=2)
fagccz <- round((mean(fixef(longtemps.full, pars="cc.z", summary=FALSE))+mean(fixef(longtemps.full, pars="speciesFAGSYL", summary=FALSE))), digits=3)
fraccz <- round((mean(fixef(longtemps.full, pars="cc.z", summary=FALSE))+mean(fixef(longtemps.full, pars="speciesFRAEXC", summary=FALSE))), digits=2)
queccz <- round((mean(fixef(longtemps.full, pars="cc.z", summary=FALSE))+mean(fixef(longtemps.full, pars="speciesQUEROB", summary=FALSE))), digits=2)
...
naocc<-round((mean(fixef(longtemps.full, pars="nao.z", summary=FALSE)+mean(fixef(longtemps.full, pars="nao.z:cc.z", summary=FALSE)))/4)/(sd(fs$nao)*2)*100*0.3, digits=2) 
matcc<-round((mean(fixef(longtemps.full, pars="mat.z", summary=FALSE)+mean(fixef(longtemps.full, pars="mat.z:cc.z", summary=FALSE)))/4)/(sd(fs$mat)*2)*100*2, digits=2) 
distcc<-round((mean(fixef(longtemps.full, pars="dist.z", summary=FALSE)+mean(fixef(longtemps.full, pars="dist.z:cc.z", summary=FALSE)))/4)/(sd(fs$distkm)*2)*100*150, digits=2) 
elevcc<-round((mean(fixef(longtemps.full, pars="elev.z", summary=FALSE)+mean(fixef(longtemps.full, pars="elev.z:cc.z", summary=FALSE)))/4)/(sd(fs$elev)*2)*100*200, digits=2)

### Same rule but for DVR:
matdvr<-round((mean(fixef(dvrtemps.full, pars="mat.z", summary=FALSE))/4)/(sd(fsdvr$mst)*2)*100*2, digits=2) ## -8.078 
naodvr<-round((mean(fixef(dvrtemps.full, pars="nao.z", summary=FALSE))/4)/(sd(fsdvr$nao)*2)*100*0.3, digits=2)## 2.04 
distdvr<-round((mean(fixef(dvrtemps.full, pars="dist.z", summary=FALSE))/4)/(sd(fsdvr$distkm)*2)*100*150, digits=2) ## 5.36
elevdvr<-round((mean(fixef(dvrtemps.full, pars="elev.z", summary=FALSE))/4)/(sd(fsdvr$elev)*2)*100*200, digits=2) ## 1.77 
ccdvr<-round((mean(fixef(dvrtemps.full, pars="cc.z", summary=FALSE))/4)/(sd(fsdvr$cc)*2)*100, digits=2) ## 8.6

### Okay now, we can calculate the interaction effects:
naoccdvr<-round((mean(fixef(dvrtemps.full, pars="nao.z", summary=FALSE)+mean(fixef(dvrtemps.full, pars="nao.z:cc.z", summary=FALSE)))/4)/(sd(fsdvr$nao)*2)*100*0.3, digits=2)
matccdvr<-round((mean(fixef(dvrtemps.full, pars="mat.z", summary=FALSE)+mean(fixef(dvrtemps.full, pars="mat.z:cc.z", summary=FALSE)))/4)/(sd(fsdvr$mat)*2)*100*2, digits=2) 
distccdvr<-round((mean(fixef(dvrtemps.full, pars="dist.z", summary=FALSE)+mean(fixef(dvrtemps.full, pars="dist.z:cc.z", summary=FALSE)))/4)/(sd(fsdvr$distkm)*2)*100*150, digits=2)
elevccdvr<-round((mean(fixef(dvrtemps.full, pars="elev.z", summary=FALSE)+mean(fixef(dvrtemps.full, pars="elev.z:cc.z", summary=FALSE)))/4)/(sd(fsdvr$elev)*2)*100*200, digits=2) 


### Same rule but for Five model:
matlong<-round((mean(fixef(long.full, pars="mat.z", summary=FALSE))/4)/(sd(fslong$mst)*2)*100*2, digits=2) ## -11.56
naolong<-round((mean(fixef(long.full, pars="nao.z", summary=FALSE))/4)/(sd(fslong$nao)*2)*100*0.3, digits=2) ## 1.26
distlong<-round((mean(fixef(long.full, pars="dist.z", summary=FALSE))/4)/(sd(fslong$distkm)*2)*100*150, digits=2) ## 2.75
elevlong<-round((mean(fixef(long.full, pars="elev.z", summary=FALSE))/4)/(sd(fslong$elev)*2)*100*200, digits=2) ## 7.35
cclong<-round((mean(fixef(long.full, pars="cc.z", summary=FALSE))/4)/(sd(fslong$cc)*2)*100, digits=2) ## 14.55
alllongccstand<-((0.58)+(0.58+0.1)+(0.58+0.07)+(0.58-0.34)+(0.58-0.47)+(0.58-0.35))/6
alllongcc<-round((alllongccstand/4)/(sd(fslong$cc)*2)*100, digits=2)

### Okay now, we can calculate the interaction effects:
naocclong<-round((mean(fixef(long.full, pars="nao.z", summary=FALSE)+mean(fixef(long.full, pars="nao.z:cc.z", summary=FALSE)))/4)/(sd(fslong$nao)*2)*100*0.3, digits=2) 
matcclong<-round((mean(fixef(long.full, pars="mat.z", summary=FALSE)+mean(fixef(long.full, pars="mat.z:cc.z", summary=FALSE)))/4)/(sd(fslong$mat)*2)*100*2, digits=2)
distcclong<-round((mean(fixef(long.full, pars="dist.z", summary=FALSE)+mean(fixef(long.full, pars="dist.z:cc.z", summary=FALSE)))/4)/(sd(fslong$distkm)*2)*100*150, digits=2)
elevcclong<-round((mean(fixef(long.full, pars="elev.z", summary=FALSE)+mean(fixef(long.full, pars="elev.z:cc.z", summary=FALSE)))/4)/(sd(fslong$elev)*2)*100*200, digits=2)
