...
disttempsz<-round((mean(fixef(fivelong.full, pars="dist.z", summary=FALSE))), digits=2)
elevtempsz<-round((mean(fixef(fivelong.full, pars="elev.z", summary=FALSE))), digits=2)
...
After climate change, the effects of these climatic and geographic factors on false spring risk shifted (Figure \ref{fig:maineffects}). With climate change, the effect of mean spring temperature on false spring risk remained consistent where warmer sites still tended to have lower risks of false springs \Sexpr{matcc}\% in risk per 2$^{\circ}$C (or \Sexpr{matccz} $\pm$ 0.06 probability of false spring/standard unit versus \Sexpr{mat}\% per 2$^{\circ}$C or \Sexpr{matz} before climate change; Figure \ref{fig:maineffects} and Figure \ref{fig:suppapc}\textbf{a}). The level of risk remained consistent before and after 1983 at sites further from the coast (Figure \ref{fig:maineffects} and Figure \ref{fig:suppapc}\textbf{b}). With warming, there was a large reduction in risk in false springs at higher elevations (Figure \ref{fig:maineffects} and Figure \ref{fig:suppapc}\textbf{c}), with \Sexpr{elevcc}\% increase in risk per 150km (or \Sexpr{elevccz} $\pm$ 0.06 probability of risk/standard unit versus \Sexpr{elev}\% increase 150km or \Sexpr{elevz} $\pm$ 0.08 before climate change). After climate change, the rate of false spring incidence largely decreased with increasing NAO indices (Figure \ref{fig:maineffects} and Figure \ref{fig:suppapc}\textbf{d}), higher with a \Sexpr{naocc}\% in risk per 0.3 unit increase in the NAO index (or \Sexpr{naoccz} $\pm$0.06 probability of false spring/standard unit or versus \Sexpr{nao}\% per 0.3 unit increase in the NAO index or \Sexpr{naoz} $\pm$ 0.06 before climate change). After climate change, NAO had the strongest effect on false spring risk, with higher NAO indices rendering fewer false springs.

Overall, there little change in false spring risk across all species (\Sexpr{allcc}\% or \Sexpr{allccstand} in probability or risk/standard unit), captured by the climate change predictor, which represents remaining variability unexplained by the climatic and geographic factors after 1983. This residual effect of climate change varied by species, with an \Sexpr{aescc}\% increased risk in false springs after climate change for \textit{Aesculus hippocastanum} (or \Sexpr{aesccz} $\pm$ 0.06 probability of false spring/standard unit; Figure \ref{fig:maineffects}, Figure \ref{fig:spp}\textbf{d} and Table \ref{tab:suppmodlong}), a \Sexpr{alncc}\% increase for \textit{Alnus glutinosa}, and a \Sexpr{betcc}\% increase for \textit{Betula pendula} (or a \Sexpr{alnccz} $\pm$ 0.09 and \Sexpr{betccz} $\pm$ 0.07 probability of false spring/standard unit respectively; Figure \ref{fig:maineffects}, Figure \ref{fig:spp}\textbf{e} and Table \ref{tab:suppmodlong}). Climate change decreased risk for \Sexpr{fagcc}\% for \textit{Fagus sylvatica}, \textit{Fraxinus excelsior} by \Sexpr{fracc}\% and \textit{Quercus robur} by \Sexpr{quecc}\% (or \Sexpr{fagccz} $\pm$ 0.09, \Sexpr{fraccz} $\pm$ 0.11 and \Sexpr{queccz} $\pm$ 0.09 probability of false spring/standard unit respectively; Figure \ref{fig:maineffects}, Figure \ref{fig:spp}\textbf{e} and Table \ref{tab:suppmodlong}).  

\subsection*{Sensitivity of results to duration of risk and temperature thresholds}
Our results remained consistent (in direction and magnitude) when we applied different rates of leafout for each species (i.e., varied the length of time between estimated budburst and leafout). Mean spring temperature (\Sexpr{matdvr}\% for every 2$^\circ$C or \Sexp{matdvrz} $\pm$ 0.06 probability of risk/standard unit), distance from the coast (\Sexpr{distdvr}\% increase for every 150km or \Sexp{distdvrz} $\pm$ 0.07 probability of risk/standard unit), elevation (\Sexpr{elevdvr}\% increase for every 200m or \Sexp{elevdvrz} $\pm$ 0.07 probability of risk/standard unit) and nao (\Sexpr{naodvr}\% increase for every 0.3 or \Sexp{naodvrz} $\pm$ 0.05 probability of risk/standard unit) all contributed to false spring risk (Figure \ref{fig:dvr} and Table \ref{tab:suppmoddvr}). After climate change, remained consistent with our main findings (Figure \ref{fig:dvr} and Table \ref{tab:suppmoddvr}). 

Results remained generally consistent also when we applied a lower temperature threshold for defining a false spring (i.e., -5$^{\circ}$C), though there were more shifts in the magnitude of some effects, especially those of climate change. Mean spring temperature (\Sexpr{matfive}\% for every 2$^\circ$ or \Sexpr{matfivez} $\pm$ 0.12 probability of risk/standard unit) was the strongest predictor but distance from the coast (\Sexpr{distfive}\% increase in risk for every 150km or \Sexpr{distfivez} $\pm$ 0.13 probability of risk/standard unit), elevation (\Sexpr{elevfive}\% increase in risk for every 200m or \Sexpr{elevfivez} $\pm$ 0.14 probability of risk/standard unit) and NAO (\Sexpr{naofive}\% increase in risk for every 0.3 or \Sexpr{naofivez} $\pm$ 0.12 probability of risk/standard unit) all contributed to risk of false spring. There was much greater increase in false spring risk due to the residual climate change effect across all six species (\Sexpr{allfivecc}\% increase or \Sexpr{allfiveccstand} $\pm$ 0.11 probability of risk/standard unit; Figure \ref{fig:five} and Table \ref{tab:suppmodfive}). 
